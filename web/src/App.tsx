...
function planningPeriod(months: OrganizationMonth[]) {
  return `${monthLabel(months[0].month, true)}–${monthLabel(endMonth(months).month, true)}`;
}

function isActiveOverride(override: ScenarioRoleOverride, role: RoleProvenance) {
  return override.annual_expected_attrition_rate !== role.annual_expected_attrition_rate
    || override.staffing_targets.some((target, index) => target !== role.staffing_targets[index]);
}
...
function StaffingTrajectory({ baseline, scenario, optimized }: { baseline: OrganizationMonth[]; scenario?: OrganizationMonth[]; optimized?: OrganizationMonth[] }) {
  ...
  return (
    <figure className="chart-card" aria-labelledby="trajectory-heading">
      <figcaption><h2 id="trajectory-heading">Organization staffing trajectory</h2><p>Expected FTE compared with staffing target for the fixed synthetic demo planning horizon.</p></figcaption>
...
function SummaryCard({ label, total, ending, spend }: { label: string; total: number; ending: OrganizationMonth; spend?: number }) {
  return <article className="summary-card"><p>{label}</p><strong>{fteMonths(total)}</strong><span>understaffed FTE-months</span><dl><div><dt>End-of-horizon expected FTE</dt><dd>{number.format(ending.expected_fte)}</dd></div><div><dt>End-of-horizon target</dt><dd>{number.format(ending.staffing_target)}</dd></div><div><dt>End-of-horizon shortage</dt><dd>{number.format(ending.staffing_shortage)}</dd></div>{spend !== undefined && <div><dt>Incremental spend</dt><dd>{currency.format(spend)}</dd></div>}</dl></article>;
}
...
export default function App() {
  ...
  useEffect(() => {
    fetchProductionForecast()
      ...
      .catch((caught: unknown) => setError(caught instanceof Error ? caught.message : "Unable to load the synthetic demo baseline."))
  ...
  const roleOverrides = useMemo(() => Object.values(overrides).filter((override) => {
    const role = baseline?.role_provenance.find((candidate) => candidate.role_id === override.role_id);
    return role ? isActiveOverride(override, role) : false;
  }), [baseline, overrides]);
  ...
  return <main className="app-shell">
    <header className="hero">
      ...
      <p>Compare the persisted demo baseline, a transient planning scenario, and an optimized hiring plan for the {demoPlanningPeriod} demo planning horizon.</p>
    </header>

    {error && <p className="error" role="alert">{error}</p>}

    <section className="baseline-panel" aria-labelledby="baseline-heading">
      <div><p className="section-kicker">1. Demo baseline</p><h2 id="baseline-heading">What does this fixed simulation show?</h2><p>Observed through {monthLabel(baseline.observation_date, true)}; fixed synthetic planning simulation for {demoPlanningPeriod}. No scenario edits or new optimized hires.</p><p className="fte-definition"><strong>About FTE-months:</strong> 1 understaffed FTE-month means being short one full-time employee for one month. Example: 2 FTE short for 3 months = 6 FTE-months. Lower is better.</p></div>
      <div className="baseline-stat"><strong>{fteMonths(baseline.total_understaffed_fte_months)}</strong><span>demo-horizon understaffed FTE-months</span></div>
    </section>
    ...
    <section className="content-grid">
      <aside className="scenario-panel" aria-labelledby="scenario-heading">
        ...
        <p className="helper">Changes are transient. They never write to the persisted synthetic demo baseline.</p>
        ...
        {activeOverrideRoles.length > 0 && <div className="active-edits" aria-live="polite"><p>Scenario includes edits to {activeOverrideRoles.length} {activeOverrideRoles.length === 1 ? "role" : "roles"}:</p><ul>{activeOverrideRoles.map((role) => <li key={role.role_id}>{role.department} · {role.role}</li>)}</ul></div>}
        ...
        <div className="constraint-grid"><label htmlFor="budget">Planning-period incremental workforce budget<input id="budget" aria-label="Planning-period incremental workforce budget" type="number" min="0" step="1000" value={budget} onChange={(event) => { setBudget(Number(event.target.value)); setOptimized(null); }} /></label><fieldset><legend>Monthly recruiting capacity</legend><div className="capacity-grid">{capacity.map((value, index) => <label key={index}>{planningMonth(index)}<input aria-label={`Recruiting capacity ${planningMonth(index)}`} type="number" min="0" step="1" value={value} onChange={(event) => { const next = [...capacity]; next[index] = Number(event.target.value); setCapacity(next); setOptimized(null); }} /></label>)}</div></fieldset></div>
    ...
    <section className="lower-grid">
      <section className="table-panel" aria-labelledby="departments-heading"><p className="section-kicker">Projected gaps</p><h2 id="departments-heading">Largest end-of-horizon department shortages</h2><table><thead><tr><th>Department</th><th>Expected FTE</th><th>Target</th><th>Shortage</th></tr></thead><tbody>{finalDepartmentShortages((optimized?.optimized_department_months ?? scenario?.department_months ?? baseline.department_months)).map((row) => <tr key={`${row.department}-${row.month}`}><td>{row.department}</td><td>{number.format(row.expected_fte)}</td><td>{number.format(row.staffing_target)}</td><td><span className="gap-value">{number.format(row.staffing_shortage)}</span></td></tr>)}</tbody></table></section>
      <section className="explain-panel" aria-labelledby="explain-heading"><p className="section-kicker">Decision logic</p><h2 id="explain-heading">What this plan optimizes</h2>{optimized ? <><p>The optimizer considers feasible combinations of integer hire starts. It first minimizes total understaffed FTE-months, subject to staffing targets, hiring lead times, monthly recruiting capacity, and the submitted planning-period incremental workforce budget.</p><p>Among plans with the same understaffing result, it minimizes in-horizon incremental workforce spend.</p>{largestRecommendation ? <p><strong>{largestRecommendation.department} · {largestRecommendation.role}</strong> receives {largestRecommendation.hires} {largestRecommendation.hires === 1 ? "hire" : "hires"} as part of the globally optimized combination.</p> : <p>No additional in-horizon hire improves the scenario under the submitted constraints.</p>}{optimized.unused_budget > 0 && <p>{currency.format(optimized.unused_budget)} remains unused because no additional in-horizon hire improves the objective under the submitted staffing targets, capacity, and lead times.</p>}<p>This is not a business-priority ranking, and the model has no standalone role-importance score.</p><p className="model-limit">The model does not include role criticality, revenue impact, skill scarcity, employee or candidate quality, or strategic priority weights.</p></> : <p>Run a scenario and optimize it to see a deterministic explanation based on the submitted assumptions and actual solver output.</p>}</section>
    </section>
